    ...
    fn heading() {
        for direction in Direction::iter() {
            for a in 0usize..=5 {
                ...
                assert_eq!(anchor_edge_pos.heading(), direction.invert());
                assert_eq!(anchor_edge_neg.heading(), direction.invert());
                ...
                for b in 0usize..=5 {
                    ...
                    assert_eq!(anchor_cell_pp.heading(), direction);
                    assert_eq!(anchor_cell_pn.heading(), direction);
                    assert_eq!(anchor_cell_np.heading(), direction);
                    assert_eq!(anchor_cell_nn.heading(), direction);
                }
            }
        }
    }
            ...
            for x_length in 0usize..=9 {
                for y_length in 0usize..=9 {
                    for a in 0usize..=5 {
                        ...
                        let (co_x_pos, co_y_pos, co_x_neg, co_y_neg) = match direction {
                            Direction::North => (co_pos, 0.into(), co_neg, 0.into()),
                            Direction::South => (co_pos, Coordinate::Pos(y_length) - 1, co_neg, Coordinate::Pos(y_length) - 1),
                            Direction::West => (0.into(), co_pos, 0.into(), co_neg),
                            Direction::East => (Coordinate::Pos(x_length) - 1, co_pos, Coordinate::Pos(x_length) - 1, co_neg),
                        };

                        let exp_pos_p = Position::new(co_x_pos, co_y_pos);
                        let exp_pos_n = Position::new(co_x_neg, co_y_neg);

                        assert_eq!(anchor_edge_pos.position(x_length, y_length), exp_pos_p);
                        assert_eq!(anchor_edge_neg.position(x_length, y_length), exp_pos_n);
                        ...
                        for b in 0usize..=5 {
                            ...
                            assert_eq!(anchor_cell_pp.position(x_length, y_length), pos_pp);
                            assert_eq!(anchor_cell_pn.position(x_length, y_length), pos_pn);
                            assert_eq!(anchor_cell_np.position(x_length, y_length), pos_np);
                            assert_eq!(anchor_cell_nn.position(x_length, y_length), pos_nn);
                        }
                    }
                }
            }
